...
with open("all_runs_step_by_step.txt", "a", encoding="utf-8") as step_by_step:
    step_by_step.write("all_runs_step")
...
def creating_relvals(ticket_prepid):
    ...
    with open("relvals_for_pushing.txt", "w", encoding="utf-8") as relvals_for_pushing:
        print("Ticket prepid for out proba.py" + ticket_prepid)
        relvals_for_pushing.write(ticket_prepid)
    time.sleep(1*60)

    with open('proba.py', 'r', encoding="utf-8") as relval_pushing_code:
    ...
    #     try:                                                                      OVAJ DIO JE PREBITAN
    #         relval_next = RelVal()
    #         print("Type: ", type(rv))
    #         print("Value: ", rv)
    #         relval_next.next_status(rv)
    ...
    ticket_relvals = relval.get('relvals', query='ticket=' + ticket_prepid)
    # open("relvals_for_testing.txt", "w").write("")
